...
export default function TrackLoop() {
	...
	for (let i = 1; i < last; i++) {
		straights.push(<Straight key={`s-${i}`} position={[pos(i), 0, pos(0)]} rotation={Math.PI / 2} />)
		straights.push(<Straight key={`n-${i}`} position={[pos(i), 0, pos(last)]} rotation={Math.PI / 2} />)
		straights.push(<Straight key={`w-${i}`} position={[pos(0), 0, pos(i)]} />)
		straights.push(<Straight key={`e-${i}`} position={[pos(last), 0, pos(i)]} />)
	}
