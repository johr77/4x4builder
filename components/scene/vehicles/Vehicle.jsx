...
const Vehicle = ({ spawnPosition = [0, 0, 0], spawnRotation = [0, 0, 0] }) => {
	...
	const colliderArgs = useMemo(() => [.25,.14,(wheelbase / 2 + axleHeight) * .23], [wheelbase, axleHeight])
	// colliderPosition
	// 1st (0),left / right,Keep 0,—
	// 2nd (1),up / down,Center of the box. (.31)
	// 3rd (0),forward / back,Keep 0,—
	const colliderPosition = useMemo(() => [0,.25 ,0])
	...
	return (
		<>
			<RigidBody ref={chassisRef} type='dynamic' position={spawnPosition}
				...
				<CuboidCollider args={colliderArgs} position={colliderPosition} mass={100} />
