...
const Vehicle = ({ spawnPosition = [0, 0, 0], spawnRotation = [0, 0, 0] }) => {
	...
	return wheelPositions.map((wheel, i) => ({
		...
		suspensionRestLength: 0.11,
		//suspensionRestLength: 0.1,
		suspensionStiffness: 17,
		//suspensionStiffness: 28,
		radius: (config.tire_diameter * 2.65) / 100 / 2,
	...
	// Collider props
	// colliderArgs
	// 1st (0.9),half width,Left–right.(.35)
	// 2nd (0.5),half height,Up–down. Full height = 1.0,(.14)
	// 3rd (wheelbase / 2 + axleHeight),half length,Front–back,(*.35)
	// Bigger = nose/tail hit jumps and ramps sooner
	const colliderArgs = useMemo(() => [.42,.2,(wheelbase / 2 + axleHeight) * .4], [wheelbase, axleHeight])
	// colliderPosition
	// 1st (0),left / right,Keep 0,—
	// 2nd (1),up / down,Center of the box. (.31)
	// 3rd (0),forward / back,Keep 0,—
	const colliderPosition = useMemo(() => [0,.42 ,0])
	...
	return (
		<>
			<RigidBody ref={chassisRef} type='dynamic' position={spawnPosition}
				...
				<CuboidCollider args={colliderArgs} position={colliderPosition} mass={6} />
